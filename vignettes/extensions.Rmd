...
3. [Method Detail](#method-details): in-depth look at the key [`ggproto`
   methods](#ggprot-method) used to generate the plot.
...
each [`Layer` object](#layer) there is an additional level of implicit
structure we did not show above. The [`ggplot_build`](#ggplot-build) process
...
Layer-related [`ggproto` objects](#ggproto) reflect this by their
`$*_layer`, `$*_panel`, and `$*_group` [methods](#ggproto-method).
...
facets, etc.  The work is carried out by the methods of [`ggproto`
objects](#ggproto) provided implicitly via the [plot specification](#plot-spec).
...
Assigns a panel, row, column, and [position scale](#ggroto-scales-position) ids
to each facet.  It is possible to have multiple [position
scales](#ggroto-scales-position) in a plot when the scales are "free".
...
For each layer, [map](#scale-map) data with [position scales](#position-scale).
...
`data` parameter for `$compute_panel` will look like (for the first panel):
...
`StatBin`), the input into `$compute_group` will look the same as the input into
`$compute_panel` because there is only one group in the first panel.  If we had
multiple groups then `$compute_group` would be called repeatedly with the subset
of rows corresponding to each group.
...
this generally produces the desired outcome.
...
scales](#ggroto-scales-position) in `ggplot_build`) across every layer.

`scales_map_df` [maps](#map-scale) every aesthetic corresponding to the provided
scales (non-[position scales](#ggroto-scales-position) in `ggplot_build`) across
every layer.
...
These methods are responsible for [rendering](#render) applying a statistic to
the [layer data](#layer-data) the [layer data](#layer-data) into [grobs](#grob).
Typically you would modify one of the layer, panel, or group methods, listed
here in order of increasing specificity.  If possible, you should modify the
most specific method (e.g. the group method) as that requires the least work.
If you need more control over the process you can override one of the more
general methods.  If you do this, for example by overriding the panel method,
you will be responsible for splitting the data into groups, processing the
groups, and re-assembling the result
...
```{r echo=FALSE}
structure(list(fill = c("#F8766D", "#00BA38"), x = c(-0.652728859521449, 
-2.3874772593379), y = c(5.21457787621766, 5.85442789699882), 
...
```
List of 7
 $ x   : 'unit' num [1:2] 0.9545native 0.0455native
  ..- attr(*, "valid.unit")= int 4
  ..- attr(*, "unit")= chr "native"
 $ y   : 'unit' num [1:2] 0.361native 0.4native
  ..- attr(*, "valid.unit")= int 4
  ..- attr(*, "unit")= chr "native"
 $ pch : int [1:2] 21 21
 $ size: 'unit' num 1char
  ...
  ..- attr(*, "unit")= chr "char"
 $ name: chr "geom_point.points.481"
 $ gp  :List of 4
...
```{r, echo=FALSE, results="asis"}
coord.trans.in <- structure(list(fill = c("#F8766D", "#00BA38"), x =
c(-1.11671940125525, -1.77017053775489), y = c(4.79062976539135,
  6.30908769629896), PANEL = structure(c(1L, 1L), .Label = c("1", "2"), class =
  "factor"), group = 1:2, shape = c(21, 21), colour = c("black", "black"), size
  = c(4, 4), alpha = c(NA, NA), stroke = c(0.5, 0.5)), class = "data.frame",
  row.names = c(NA, -2L), vars = "PANEL")
coord.trans.out <- structure(list(fill = c("#F8766D", "#00BA38"), x =
c(0.848436907949773, 0.254390420222824), y = c(0.335795743357051,
  0.427823496745391), PANEL = structure(c(1L, 1L), .Label = c("1", "2"), class =
  "factor"), group = 1:2, shape = c(21, 21), colour = c("black", "black"), size
  = c(4, 4), alpha = c(NA, NA), stroke = c(0.5, 0.5)), row.names = c(NA, -2L),
  vars = "PANEL", class = "data.frame")
...
aesthetic are reduced in with and placed side by side to fit where one of them
...
[coordinate transformation](#coordtransform) instead.
</dd>
<!---------------------------------------------------------------------------->
