...
[`Position`](#ggproto-geom), `Scale`, and/or `Facet` `ggproto` objects.
...
Here is a simplified version of the layer function `geom_point`:
...
The layer  function calls `ggplot2::layer`. This does two non-obvious things:
expand shortcut names for `ggproto` objects into the corresponding `ggproto`
object, and allocate its other parameters to the different components.  In
...
* `Layout$setup`:
    ...
    * [`Coord$setup_data`](#setup_params-setup_data): unused by common `Coord`
      objects.
    * [`Facet$compute_layout`](#facet-compute_layout): facet meta data.
    * `Coord$setup_layout`:
...
* `Layer$compute_statistic`: recompute data with statistics.
    ...
    * `Stat$compute_panel`: apply statistic to each panel
        * `Stat$compute_group`: apply statisitc to each group
...
This is what this data looks like for our example:
...
A [`ggproto` method](#ggproto-method) that provides an opportunity to check that
parameters for a stat are correct, and to update them as necessary, for example
to provide defaults that need to be computed from the data.  It is run for each
layer in a plot.
...
* `self`: optional parameter, if present will point to the [`Stat`
  object](#stat-ggproto) the [`ggproto` method](#ggproto-method) belongs to.
* `data`: the layer data with computed and scale-transformed aesthetics, and
  with "mapped" [position aesthetics](#position-aesthetic).
...
<dd>
  Functions that create [`Layer` `ggproto`](#ggprot0-layer)) objects like
  `geom_point` or `stat_bin`.  These functions will allocate their parameters to
