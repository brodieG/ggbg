...
* `Facet$setup_data`:
* `Coord$setup_data`:
* `Facet$compute_layout`: assign `PANEL`, row, col, and x/y (position) scale ids
  to each facet (there are different scales when scales are "free").
* `Coord$setup_layout`:
* `Facet$map_data`: add the `PANEL` variable that maps data rows to facets.
* For each layer:
    * `Layer$compute_aesthetics`: evaluate the non-`stat` expressions in `aes()`
      and if necessary infer the `group` variable from categorical aesthetics.
* For each layer:
    * `Scale$transform_df`: apply scale transformations to aesthetics (e.g. log
      tranformation).
* `Layout$train_position`: initialize scales
* For each layer:
    * For each facet: rescale position data (e.g. x, xmin, xmax, xintercept, y,
      ymin, ymax, yintercept)???? Doesn't seem to actually happen here.
...
library(ggplot2)
p <- ggplot(dat, aes(x=location, y=value, fill=grp)) +
  stat_bin2d() +
