...
1. [Overview](#overview): A high level look at the process of converting data
   ...
   context for the reference sections that follow.
...
[`Scale`](#ggproto-scale), [`Coord`](#ggroto-coord), [`Facet`](#ggproto-facet),
...
This structure implies several things about a plot:
...
Implicit in this structure is that every plot [layer](#layer) maps onto the same
space.

The portion of the structure you are most likely to interact with when extending
`ggplot2` is the `Layer` object and its sub-objects.  Within each `Layer` object
there is an additional level of implicit structure. The
...
that [panel](#panel).  The pseudo structure of a `Layer` is then:
...
<!---------------------------------------------------------------------------->
<dt><a name='ggprot-coord'></a>Coord, `ggproto`</dt>
<dd>
...
Facets are sub-viewports in which a subset of the data is displayed.  Each plot
has a set of facets, or by default a single facet that corresponds to the main
viewport.  Each plot [layer](#layer) is displayed across all plot facets,
although some layers may not have data to show in every facet.

The area of a facet that is wholly bounded by the axes (i.e. excluding axis
labels, etc.) is the [panel](#panel).
<dd>
<!---------------------------------------------------------------------------->
...
One special feature of `ggproto` methods is that they can have a special `self`
...
opportunity to modify the `ggproto` object by reference.
...
```{r}
ggplot2::GeomPoint$setup_data
environment(ggplot2::GeomPoint$setup_data)$f
```
...
If you can build the code using `ggproto` methods it will probably be
easier to add a `browser` call to the sources.
</dd>
<!---------------------------------------------------------------------------->
...
Internally, layers and the logic required to compute and render them are built
into the [`Layer` `ggproto`](#ggproto-layer) object.
</dd>
<!---------------------------------------------------------------------------->
<dt><a name='ggproto-layer'></a>Layer, `ggproto`</dt>
<dd>
[`ggproto`](#ggproto) object responsible for computing and rendering plot
[layers](#layer).  It contains `Geom`, `Stat`, and `Position` `ggproto`
sub-objects, along with other data and the methods that carry out the
computation and translation of layer data into [grobs](#grob).
</dd>
<!---------------------------------------------------------------------------->
...
* `Layout$get_scales`: a method which when given a panel id will return a list
  with the position scales associated with that panel.
...
* Translate shorthand geom/stat/position strings into the actual [ggproto
  objects](#ggproto).
* Assign parameters to the [`Layer`](#ggproto-layer) object and sub-objects.

To illustrate consider the following call to a layer function:
```{r eval=FALSE}
layer <- ggplot2::stat_bin(geom='point', shape=21, bins=10)
```
The layer function converts the shorthand "point" into a `GeomPoint` `ggproto`
sub-object and assigns it the `shape` parameter, and assigns the "bins"
parameter to the `StatBin` `ggproto` sub-object:
```{r}
class(layer$geom)
layer$geom_params
layer$stat_params$bins
```

Parameter assignment is done by matching against the
`Geom`/`Stat` `$[compute|draw]_[panel|group]` methods, and against the `Geom`
aesthetics.  For example, `StatBin` has both `$compute_panel` and
`$compute_group` methods.  The layer function will look at the parameters of the
`$*_panel` method, and if that method has a `...` parameter as is the case here,
it will also look at the parameters of the `$*_group` method.
...
Because of the `...` in the `$compute_panel` method, the `stat_bin` layer
function will match parameters to the union of the `$compute_panel` and
`$compute_group` methods argument lists.  Matched parameters will then be stored
as the `$parameters` member of the [`Stat`](#ggproto-stat) object for use by
`$compute_panel` and `$compute_group` in the [stat computation
step](#stat-compute).


A similar process occurs with `GeomPoint` and the `$draw_panel` methods.


Most of the work a layer function does is translate  and assign their parameters 

The key role of the 
kkkkk
like `geom_point` or `stat_bin`.  For example, in (a simplified version
of) `geom_point`:
...
shortcut name into a `PositionJitter` [`ggproto` object](#ggproto).  The string
"identity" is translated into `StatIdentity` and `PositionIdentity` for the
`stat` and `position` parameters respectively.

In addition to expanding shortcut names into [`ggproto` objects](#ggproto),
layer functions allocate their other parameters to those `ggproto` objects, or
or alternatively to the aesthetics as a [parameter
aesthetic](#parameter-aesthetic).  </dd>
<!---------------------------------------------------------------------------->
...
panel/[facet](#facet) each row in the layer data should be displayed in.
</dd>
<!---------------------------------------------------------------------------->
