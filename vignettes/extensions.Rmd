...
(note we are actually showing the effect of `ggplot2:::scales_transform_df`, a
wrapper around the `$transform_df` method):
...
that are associated with the scale.

You can check which aesthetics are
associated with a scale with the `$aesthetics` member of the scale:
...
numbers negative, effectively reversing them.
...
```{r echo=FALSE}
stat.in <- structure(list(x = c(-1, -2, -3, -4), weight = 5:8, PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group = c(-1L, -1L, -1L, -1L)), row.names = c(NA, -4L), class = "data.frame")
```
...
In this case, because we only have one group, if you did not modify the panel
method (as is the case with `StatBin`), the input into `$compute_group` will look
the same as the input into `$compute_panel`.  If we had multiple groups then
`$compute_group` would be called repeatedly with the subset of rows
...
```{r echo=FALSE}
structure(list(count = 11, x = -2.5, xmin = -4, xmax = -1, width = 3, density = 0.333333333333333, ncount = 1, ndensity = 1), class = "data.frame", row.names = c(NA, -1L))
```
...
```{r echo=FALSE}
stat.out <- structure(list(count = c(11, 15), x = c(-2.5, -2.5), xmin = c(-4, -4), xmax = c(-1, -1), width = c(3, 3), density = c(0.333333333333333, 0.333333333333333), ncount = c(1, 1), ndensity = c(1, 1), PANEL = structure(1:2, .Label = c("1", "2"), class = "factor"), group = c(-1L, -1L)), class = "data.frame", row.names = c(NA, -2L))
```
...
```{r echo=FALSE, results="asis"}
diffobj::diffStr(stat.in, stat.out)
```
...
object, assigns the `shape` parameter to the [aesthetic
parameter](#parameter-aesthetic) list, and assigns the `bins` parameter to the
