...
2. Build (`ggplot_build`): `ggplot2` manipulates the data as directed by the
   user plot specification (e.g. apply stats, position adjustments, etc).
3. Create Grobs (`ggplot_gtable`): `ggplot2` translates the data into `grid`
...
* [`Layer$layer_data`](#layer-layer_data): assign data to each layer.
* `Layout$setup`:
...
* `Layout$train_position`: initialize and [train](#train-scale) position
  scales.
* [`Layout$map_position`](#layout-map_position): [map](#map-scale) [position
...
* `Layer$compute_position`: apply position adjustments.
    ...
    * [`Position$compute_layer`](#position-compute_layer): adjust positions for
      layer.
    * [`Position$compute_panel`](#position-compute_panel): adjust positions for
      panel.
...
* [`Layout$setup_panel_params`](#layout-setup_panel_params): compute meta-data
  for panels.
* [`Layout$map_position`](#layout-map_position): re-map [position
  scales](#ggproto-position-scale).
...
* `Layout$finish_data`:
    * `Facet$finish_data`: typically unused, gives `Facet` one last chance to
      modify data with access to the [layout](#layout-layout) meta data and the
      position scales.
...
The data, with [computed](#computed-aesthetic) and other aesthetics specified by
...
A variable that relates to a plot property, such as `x` position, or
`size`, or `color`.

Aesthetics come about in three ways in `ggplot2`:

* <a name='mapped-aesthetic'></a>Mapped aesthetics are specified via the
  `mapping` parameter to [layer functions](#layer-function) with a call to `aes`
  (e.g. `geom_point(mapping=aes(x=x, ...))`.
* <a name='stat-aesthetic'></a>Stat aesthetics are generated by some of the
  [`Stat`](#ggproto-stat) objects in the process of transforming the layer data
  by applying the `Stat` computations.  They end up as [additional columns]() in the
  computed layer data, and can be referenced with the `stat` call in the mapping
  specification (e.g. `stat_bin(aes(y=stat(ncount)))`).

  resulting new layer data will have
  new varia

  , typically by applying statistics to the layer
  data.  These aesthetics are added to the result of 

  layers and can be used in a plot by including
  a call to `stat` in the mapping call (e.g. `geom_bar(stat=bin,
  mapping=aes(y=stat(density)))`)
* <a name='param-aesthetic'></a>


`ggplot2` allows users to specify the mapping with the
...
<dt><a name='computed-aesthetic'></a>Aesthetic, Computed</dt>
<dd>
Aesthetic variables that are generated by [`Stat`](#ggproto-stat).  These can
be referenced in the aesthetic specification by using the `stat` function
(e.g.  `aes(y=stat(count))`).  The `count` variable does not exist in the
input data, but it does exist in the data produced by the
[`Stat`](#ggproto-stat).

What computed variables are available will depend on the statistic.  Sometimes
this is documented in the [layer functions](#layer-function) that use those
`Stat` objects (e.g. `?stat_bin`), but not always.  If the variables are not
documented, you can look at the most specific
[`Stat$compute_*`](#stat-compute) that the [`Stat`](#ggproto-stat) defines.
</dd>
...
Parameter matching is typically done by looking at the parameters for the
corresponding `ggproto` `$compute/draw_panel` methods, and if that method
contains `...` in the signature, also by looking at the parameters of the
`ggproto` `$compute/draw_group` methods.  For example, for `StatBin` we have:
...
`StatBin` as the stat will match their parameters against the union of the
parameter list for `$compute_panel` and `$compute_group`.  So in the call
...
<dt><a name='ggproto-scale'></a>Scale, Ggproto</dt>
<dd>
Contains meta data about the scales (e.g. the range) and methods to transform
the data.

For example, `scale_x_sqrt` produces a `ScaleContinuous` `ggproto`
object with a `$trans` method that will covert any `x` related aesthetics to
their square roots.

There are is also typically the `$map` to map the aesthetics into the display
space (e.g. categories into colors), although for [position
aesthetics](#position-aesthetic) that is
typically handled by `Coord$transform` **This needs to be confirmed**.
</dd>
<dt><a name='ggproto-scale-position'></a>Scale, Position, Ggproto</dt>
<dd>
[Scales](#ggproto-scale) that correspond to the [position
aesthetics](#position-aesthetic).
</dd>
</dl>
<dt><a name='scale-map'></a>Scale, Map</dt>
<dd>
Use scales to convert aesthetic values into the space they will be plotted in.
...
<dt><a name='scale-train'></a>Scale, Train</dt>
<dd>
