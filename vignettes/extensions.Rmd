...
shortcut name into a `PositionJitter` `ggproto` object, and implicitly due to
the default value for `stat` parameter it also translates the string
`'identity'` into the `StatIdentity` `ggproto` object.
...
recomputing the layer data with a statistic and potentially adding new
