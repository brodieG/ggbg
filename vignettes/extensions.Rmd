...
1. [Overview](#overview): A high level look at the process of converting data
   and user `ggplot2` plot specifications into a plot.  This section provides
   context for the reference sections that follow and is the only section
   intended to be read from beginning to end.
2. [Method Outline](#method-outline): The listing of all the key function calls
   used by `ggplot2` to generate the plot.  This gives you a sense for where
   your extension fits in relation to the rest of the process.  Most steps in
   the outline are hyperlinked to the "Process Detail" section.
3. [Method Detail](#method-detail): More detail on the important steps in the
   plot generation process, including descriptions of inputs, outputs, and other
   relevant information for anyone looking to modify those steps.
4. [Definitions](#definitions): Definitions of key terms.  These are hyperlinked
...
2. Build: `ggplot2` manipulates the data as directed by the user plot
   ...
   transformations, mapping to [aesthetics](#aesthetic) etc).
...
need to review the `ggplot2` extension mechanism.  `ggplot2` is extended by
defining [`ggproto` objects](#ggproto) that inherit from the existing
[`Geom`](#ggproto-geom), [`Stat`](#ggproto-geom), [`Position`](#ggproto-geom),
[`Scale`](#ggproto-scale), [`Coord`](#ggproto-coord), [`Facet`](#ggproto-facet),
and other [`ggproto` objects](#ggproto).  [`ggproto` objects](#ggproto) contain
both data and functions (a.k.a.  [methods](#ggproto-method)).  This allows us to
override existing functionality by providing `ggproto` objects with custom
...
* Can have multiple [scales](#scale), however the set of scales is shared
...
`ggplot2` is the `Layer` object and its sub-objects.  Within each [`Layer`
object](#layer) there is an additional level of implicit structure. The
[`ggplot_build`](#ggplot-build) process adds [`PANEL`](#panel) and `group`
variables to the [`Layer$data`](#layer-data) `data.frame`.  This creates groups
of rows that each correspond to a particular [panel](#panel) or group within
that [panel](#panel).  The pseudo structure of a [`Layer`](#layer) is then:
...
As a result of this structure a lot of the processing in the `ggplot` build
process follows this pseudo code pattern:
...
This is reflected in the structure of the layer-related [`ggproto`
objects](#ggproto), which will often have `$*_layer`, `$*_panel`, and/or
`$*_group` [methods](#ggproto-method).
...
p <- ggplot(dat, aes(x=loc)) +
  stat_bin(bins=1, aes(weight=val)) +
...
possible to have multiple [position scales](#ggroto-scales-position) in a plot when the scales are "free".
...
although the effect will be the same on the second as they both share the [plot
data](#plot-data):

```{r, echo=FALSE, results="asis"}
facet.map.in <- structure(list(loc = 1:4, val = 5:8, grp = structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"), pnl = structure(c(1L, 1L, 2L, 2L), .Label = c("P1", "P2"), class = "factor")), class = "data.frame", row.names = c(NA, -4L))
facet.map.out <- structure(list(loc = 1:4, val = 5:8, grp = structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"), pnl = structure(c(1L, 1L, 2L, 2L), .Label = c("P1", "P2"), class = "factor"), PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor")), row.names = c(NA, 4L), class = "data.frame")
...
```{r, echo=FALSE, results="asis"}
comp.aes.in <- structure(list(loc = 1:4, val = 5:8, grp = structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"), pnl = structure(c(1L, 1L, 2L, 2L), .Label = c("P1", "P2"), class = "factor"), PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor")), row.names = c(NA, 4L), class = "data.frame")
comp.aes.out <- structure(list(y = 5:8, fill = structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"), x = 1:4, PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group = structure(c(1L, 2L, 2L, 3L), n = 3L)), row.names = c(NA, -4L), class = "data.frame")
...
trans.out <- structure(list(x = -1:-4, y = 5:8, fill = structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"), PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group = structure(c(1L, 2L, 2L, 3L), n = 3L)), class = "data.frame", row.names = c(NA, -4L))
...
more control over the process you can override on of the more general methods.
...
the `scales` parameter.  These parameters will be automatically matched and
...
* `layout`: the [`Layout` `ggproto`](#ggproto-layout) object for the plot,
  ...
  scales](#ggroto-scales-position) to
  pass on to `$compute_panel` and `$compute_group`.
* `scales`: a list with the [position scales](#ggproto-scales-position) for the
...
```{r echo=FALSE}
stat.comp.in <- structure(list(x = c(-1, -2, -3, -4), weight = 5:8, PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group = c(-1L, -1L, -1L, -1L)), row.names = c(NA, -4L), class = "data.frame")
```

And if you have not modified the `$compute_layer` method, then the input into
`$compute_panel` will look like (for the first panel):

```{r echo=FALSE}
structure(list(x = c(-1, -2), weight = 5:6, PANEL = structure(c(1L, 1L), .Label = c("1", "2"), class = "factor"), group = c(-1L, -1L)), class = "data.frame", row.names = c(NA, -2L), vars = "PANEL")
```

In this case, because we only have one group, if you did not modify the
`$compute_panel` method (as is the case with `StatBin`), the input into
`$compute_group` will look the same as the input into `$compute_panel`.  If we
had multiple groups then `$compute_group` would be called repeatedly with the
subset of rows corresponding to each group.
...
```{r echo=FALSE}
structure(list(count = 11, x = -1.5, xmin = -2, xmax = -1, width = 1, 
    density = 1, ncount = 1, ndensity = 1, PANEL = structure(1L, .Label = c("1", 
    "2"), class = "factor"), group = -1L), class = "data.frame", row.names = 1L)
```
...
`$compute_group`.  If you override `$compute_panel` then you will be responsible
for doing this.
...
```{r echo=FALSE}
stat.comp.out <- structure(list(count = c(11, 15), x = c(-1.5, -3.5), xmin =
c(-2, -4), xmax = c(-1, -3), width = c(1, 1), density = c(1, 1), ncount = c(1,
  1), ndensity = c(1, 1), PANEL = structure(1:2, .Label = c("1", "2"), class =
  "factor"), group = c(-1L, -1L)), class = "data.frame", row.names = c(NA, -2L))
```
...
* [`Layer$compute_aesthetic`](#layercompute-aesthetic) (kind of).
...
"2"), class = "factor"), group = c(-1L, -1L)), class = "data.frame", row.names = c(NA, -2L))
...
The data with positions adjusted.  Here is how `$compute_panel` changes the
layer data for the `geom_point` layer from our [example plot](#example-plot)
that uses `position_jitter`:

```{r, echo=FALSE, results="asis"}
pos.comp.in <- structure(list(x = c(-1, -2, -3, -4), y = c(5, 6, 7, 8), fill =
structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"), PANEL
= structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group =
structure(c(1L, 2L, 2L, 3L), n = 3L)), row.names = c(NA, -4L), class =
"data.frame")
pos.comp.out <- structure(list(x = c(-0.663717901706696, -2.17424187399447,
  -3.05681485142559, -4.28745166156441), y = c(5.13081755675375,
    6.17909746430814, 7.03512939997017, 7.87351626232266), fill =
    structure(c(1L, 2L, 2L, 3L), .Label = c("A", "B", "C"), class = "factor"),
    PANEL = structure(c(1L, 1L, 2L, 2L), .Label = c("1", "2"), class =
    "factor"), group = structure(c(1L, 2L, 2L, 3L), n = 3L)), row.names = c(NA,
      -4L), class = "data.frame")
diffobj::diffPrint(pos.comp.in, pos.comp.out)
```

This adjust the `x` and `y` aesthetics, though it would also adjust any other
[position aesthetics](#position-aesthetic) if they existed in the data.  Please
note that because the jitter is random and this vignette has been assembled
manually, `x` and `y` jittered values may vary across this document.
...
data for the [position scales](#position-scale), including the range, labels,
location of major and minor breaks, and axis order, amongst others.  The list is
then stored in `Layout$panel_params`, by [reference](#reference).  Here is what
this list looks like in our [example plot](#example-plot):

```
List of 2
 $ :List of 14
  ..$ x.range       : num [1:2] -2.07 -0.534
  ..$ x.labels      : chr [1:3] "1.0" "1.5" "2.0"
  ..$ x.major       : num [1:3] 0.6967 0.3711 0.0455
  ..$ x.minor       : num [1:6] 0.859 0.697 0.534 0.371 0.208 ...
  ..$ x.major_source: num [1:3] -1 -1.5 -2
  ..$ x.minor_source: num [1:6] -0.75 -1 -1.25 -1.5 -1.75 -2
 ...
 $ :List of 14
  ..$ x.range       : num [1:2] -4.41 -2.93
  ..$ x.labels      : chr [1:3] "3.0" "3.5" "4.0"
  ..$ x.major       : num [1:3] 0.955 0.617 0.279
  ..$ x.minor       : num [1:6] 0.955 0.786 0.617 0.448 0.279 ...
  ..$ x.major_source: num [1:3] -3 -3.5 -4
  ..$ x.minor_source: num [1:6] -3 -3.25 -3.5 -3.75 -4 -4.25
  ..$ x.arrange     : chr [1:2] "secondary" "primary"
  ..$ y.range       : num [1:2] -0.75 15.75
  ..$ y.labels      : chr [1:4] "0" "5" "10" "15"
  ..$ y.major       : num [1:4] 0.0455 0.3485 0.6515 0.9545
...
corresponding to the provided scales (non-[position scales](#ggroto-scales-position) in `ggplot_build`)
across every layer.

`scales_map_df` [maps](#map-scale) every aesthetic corresponding to the
provided scales (non-[position scales](#ggroto-scales-position) in `ggplot_build`) across every layer.

#### Value

For each layer [layer data](#layer-data), train the non-[position
scales](#position-scale) and then map the [layer data](#layer-data) with them.
...
[mapped](#mapped-aesthetic) non-[position aesthetics](#position-aesthetics)
(`fill`) so it the only modified by `scales_map_df`:

```{r, echo=FALSE, results="asis"}

map.scale.in <- structure(list(x = c(-0.864777388423681, -2.10850843768567, -3.18321079164743, 
-4.23832643423229), y = c(4.7609717823565, 5.70522983931005, 
7.06110801026225, 8.22130034621805), fill = structure(c(1L, 2L, 
...
), x = c(-0.864777388423681, -2.10850843768567, -3.18321079164743, 
-4.23832643423229), y = c(4.7609717823565, 5.70522983931005, 
7.06110801026225, 8.22130034621805), PANEL = structure(c(1L, 
1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group = structure(c(1L, 
...
defaults.in <- structure(list(fill = c("#F8766D", "#00BA38", "#00BA38", "#619CFF"
), x = c(-1.18914774078876, -2.14726227596402, -3.04320267718285, 
-4.36120297014713), y = c(5.19393369983882, 5.69687749966979, 
6.78337125554681, 7.75601978879422), 'PANEL ' = structure(c(1L, 
1L, 2L, 2L), .Label = c("1", "2"), class = "factor"), group = structure(c(1L, 
2L, 2L, 3L), n = 3L)), class = "data.frame", row.names = c(NA, 
-4L))
...
[`ggproto` methods](#ggproto-method) responsible for [rendering](#render) the
[layer data](#layer-data) into [grobs](#grob).
...
The `self` parameter, if present, can appear anywhere in the signature.  The
other parameters must be in the relative order shown above.  If you override
`$draw_panel` or `$draw_group` you may add additional parameters after
the `coord` parameter.  These parameters will be automatically matched and
...
property of a plot, such as position, color, size, and line type.  Most of the
time when we say "aesthetic" we mean "aesthetic variable".  These are variables
that contain data that is to be represented as an aesthetic on the plot.  For
example the `x` aesthetic variable will contain values that ultimately are
...
* <a name='param-aesthetic'></a>Parameter aesthetics are specified in [layer
  ...
  [mapped](#map-scale).  Instead, they are added at the end of the
  `ggplot_build` process by [`Geom$use_defaults`](#geomuse_defaults).
</dd>
<!---------------------------------------------------------------------------->
...
Each plot [layer](#layer) is displayed across all plot facets, although some
...
`ggproto` objects are [reference objects](#reference-object), so they can be
modified in place.  They are actually environments with a class attribute.
...
[reference objects](#reference-object).
...
Recall [`ggproto` objects](#ggproto) are actually environments.
</dd>
<!---------------------------------------------------------------------------->
...
<!---------------------------------------------------------------------------->
<dt><a name='grob'></a>Grid Graphical Objects</dt>
<dd>
...
simplify the manipulation of the layout of plots.
...
The `Layer` objects is not exported and as such not part of the `ggplot2`
extension API.
</dd>
<!---------------------------------------------------------------------------->
...
aesthetic](#parameter-aesthetic) list, and assigns the `bins` parameter to the
...
The `PANEL` variable in the [layer data](#layer-data) designates what
panel each row in the layer data should be displayed in.
</dd>
<!---------------------------------------------------------------------------->
...
<!---------------------------------------------------------------------------->
<dt><a name='ggproto-position'></a>Reference Object</dt>
<dd>
...
a function and modified in that function's scope will also be modified in the
...
lst$a
```
[`ggproto` objects](#ggproto) are reference objects.
</dd>
<!---------------------------------------------------------------------------->
<dt><a name='ggproto-position'></a>Render</dt>
<dd>
